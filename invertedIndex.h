...
// <char, FileListNode*>根据一个char找一个FileList
struct InvertedIndexNode {
	char *word; // key（根据word来找某一个node）
	struct FileListNode *fileList; // 在file中算出每一个file的tf，找到InvertedIndexNode，将这个file对应的FileListNode加一下
